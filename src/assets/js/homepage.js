    ...
    const generateHtml = (data) => 
    {
        ...
        for(let i=0;i<data.length;i++)
        {
            ...
            if (next_item.Conference == "NFC" && next_item.Division == "North") {
                ...
                let html = 
                `
                    <button type="button" id="${next_item.Key}" class="homeTeamButton" onClick="testFunction(this.id); testMongo(this.id);">${next_item.FullName}<img src=${next_item.WikipediaLogoUrl} class="homeTeamLogo"></button>
                `;
                html += `
                `;
